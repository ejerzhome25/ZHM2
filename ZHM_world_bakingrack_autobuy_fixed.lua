...
-- WORLD AUTO BUY - BAKING RACK PROXIMITY PROMPT
-- The Baking Rack is purchased from the world prompt shown as:
--   ObjectText: "Baking Rack (₱15000)"
--   ActionText: "Buy"
-- It is NOT a Market GUI purchase, so this scanner fires that prompt directly.
local BAKING_RACK_BUY_SCAN_INTERVAL = 0.08
local BAKING_RACK_BUY_FIRE_COOLDOWN = 0.30
local BAKING_RACK_BUY_SEARCH_RADIUS = 120

local lastBakingRackBuyFire = setmetatable({}, { __mode = "k" })

local function compactBuyText(value)
    return string.lower(tostring(value or "")):gsub("[%s_%-%./%(%)%[%]:]+", "")
end

local function ancestorMentionsBakingRack(obj)
    local current = obj
    for _ = 1, 8 do
        if not current or current == Workspace then break end
        local compact = compactBuyText(current.Name)
        if compact:find("bakingrack", 1, true)
            or compact:find("normalbakingrack", 1, true) then
            return true
        end
        current = current.Parent
    end
...
local function isBakingRackBuyPrompt(prompt)
    ...
    local action = compactBuyText(prompt.ActionText)
    local objectText = compactBuyText(prompt.ObjectText)
    local name = compactBuyText(prompt.Name)

    local isBuyAction = action == "buy"
    ...
    if not isBuyAction then
        return false
    end

    local isRackProduct = objectText:find("bakingrack", 1, true) ~= nil
        or objectText:find("normalbakingrack", 1, true) ~= nil
        or name:find("bakingrack", 1, true) ~= nil
        or name:find("normalbakingrack", 1, true) ~= nil
        or ancestorMentionsBakingRack(prompt.Parent)

    return isRackProduct
end

local function getBuyPromptPart(prompt)
    ...
    while current and current ~= Workspace do
        if current:IsA("BasePart") then
            return current
        elseif current:IsA("Attachment") and current.Parent and current.Parent:IsA("BasePart") then
...
local function findNearestBakingRackBuyPrompt()
    ...
    local bestPlotPriority = math.huge

    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and isBakingRackBuyPrompt(obj) then
            local part = getBuyPromptPart(obj)
            if part then
                local distance = (root.Position - part.Position).Magnitude
                if distance <= BAKING_RACK_BUY_SEARCH_RADIUS then
                    -- Prefer a Baking Rack prompt actually belonging to our plot.
                    -- If the buy pad is stored elsewhere, nearest matching prompt wins.
                    local plotPriority = (myPlot and obj:IsDescendantOf(myPlot)) and 0 or 1

                    if plotPriority < bestPlotPriority
                        or (plotPriority == bestPlotPriority and distance < bestDistance) then
                        bestPlotPriority = plotPriority
...
local function fireBakingRackBuyPrompt(prompt)
    if not running("ZHM_AutoBuy") then return false end
    if not prompt or not prompt.Parent or not isBakingRackBuyPrompt(prompt) then
        return false
    end

    local now = os.clock()
    local lastFire = lastBakingRackBuyFire[prompt] or 0
    if now - lastFire < BAKING_RACK_BUY_FIRE_COOLDOWN then
        return false
    end
    lastBakingRackBuyFire[prompt] = now

    pcall(function() prompt.HoldDuration = 0 end)
    pcall(function() prompt.RequiresLineOfSight = false end)
    pcall(function() prompt.MaxActivationDistance = math.max(prompt.MaxActivationDistance, 50) end)
...
local function autoBuyMarket()
    ...
    local prompt, distance = findNearestBakingRackBuyPrompt()

    if not prompt then
        report(
            "RackBuy",
            "Scanning for world Baking Rack Buy prompt within "
                .. tostring(BAKING_RACK_BUY_SEARCH_RADIUS)
                .. " studs."
        )
        task.wait(BAKING_RACK_BUY_SCAN_INTERVAL)
        return
    end
    ...
    if fireBakingRackBuyPrompt(prompt) then
        report(
            "RackBuy",
            "Baking Rack Buy prompt fired at "
...
local function startAutomation()
    ...
    for _, definition in ipairs({
        { Key = "ZHM_AutoBuy", Name = "RackBuy", Run = autoBuyMarket },
...
createToggle(extraPage, "Auto Buy Baking Rack", "Fire the world Baking Rack Buy prompt automatically", "ZHM_AutoBuy", true)
